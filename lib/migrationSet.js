...
class MigrationSet extends EventEmitter {
  ...
  constructor(config) {
    ...
    this.oracledb = new OracleDB(config);
  ...
  checkDatabaseMigrationTable() {
    return this.oracledb.checkMigrationTableExists()
      .then(() => this.load())
      .then(() => this.emit('init'))
  ...
  save(title) {
    ...
    return this.oracledb.saveMigrateData({
      title,
  ...
  /**
   * Load migration info from database and maps it
   *
   * @returns {Promise} returns Oracle DB connection
   */
  load() {
    ...
    return this.oracledb.loadMigrateData()
      .then(data => {
   ...
   */
  removeEntry(title) {
    return this.oracledb.removeMigrateEntry(title);
  }
  ...
  _migrate(direction, migrationName, cb) {
    ...
    function next(migration) {
      ...
      migration[direction]()
        .then(str => str.split(self.sqlDelimiter).map(e => e.trim()))
        .then(sqls => self.oracledb.runTransactionSequence(sqls))
        .then(() => {
          if (direction === 'up') {
            return self.save(migration.title)
              .then(() => next(migrations.shift()));
          } else if (direction === 'down') {
            return self.removeEntry(migration.title)
              .then(() => next(migrations.shift()));
          }
