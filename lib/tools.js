 ...
 * @param  {string} [NODE_ENV] optional environment, e.g. "development". If not supplied, process.env.NODE_ENV is used
 * @return {undefined}
 */
function readConfig(name, root, NODE_ENV) {
  let env = NODE_ENV || process.env.NODE_ENV;
  let config = readConfigJSON(name, root, env);

  // next try to find a config
  if(!config) {
    config = readConfigJS(`${name}.js`, root, env);
  }
 ...
 * @return {undefined}
 */
function readConfigJS(name, root, NODE_ENV) {
  assert(name, '\'name\' should be present');
  assert(root, '\'root\' should be present');
  assert(NODE_ENV, 'NODE_ENV should exist with valid value');
 ...
 * @return {undefined}
 */
function readConfigJSON(name, root, NODE_ENV) {
  assert(name, '\'name\' should be present');
  assert(root, '\'root\' should be present');
  assert(NODE_ENV, 'NODE_ENV should exist with valid value');
